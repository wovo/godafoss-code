...
class terminal:
    ...
    def write_char(
        ...
        if self._goto_state == 1:
            ...
            self._y += v
            self._goto_state = 0
            self.cursor_set( gf.xy( self._x, self._y ) )
        ...
        # handle special characters
            ...
            self.cursor_set( gf.xy( 0, self.cursor.y + 1 ) )

        elif c == '\r':
            self.cursor_set( gf.xy( 0, self.cursor.y ) )
        ...
        # handle a normal char
            ...
            self.cursor = self.cursor + gf.xy( 1, 0 )
    ...
    def clear(
        ...
        This function clears the terminal by rites a space (or another
        character) to all positions) and puts the cursor at xy(0,0).
        The default implementation does this by writing spaces to all
        locations.
        A concrete implementation might provide a better (faster) way.
        """

        for y in range( 0, self.size.y ):
            self.cursor_set( gf.xy( 0, y ) )
